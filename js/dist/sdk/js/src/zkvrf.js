...
function getOraoVRF(address, signerOrProvider) {
    return types_1.OraoVRF__factory.connect(address, signerOrProvider);
}
...
function getOrao(address, signerOrProvider) {
    return types_1.MockERC20__factory.connect(address, signerOrProvider);
}
...
class OraoVRFClient {
    ...
    orao) {
        ...
        this.request = (seed, options) => __awaiter(this, void 0, void 0, function* () {
            ...
            const subscription = yield this.vrf.getSubscription(signerAddress);
            ...
            if (config.oraoFee.gt(subscription.orao) &&
                ...
                const oraoBalance = yield this.orao.balanceOf(signerAddress);
                const allowance = yield this.orao.allowance(signerAddress, this.vrf.address);
                const oraoToPay = config.oraoFee.sub(subscription.orao);
                if (oraoBalance.gt(oraoToPay)) {
                    if (allowance.lt(oraoToPay))
                        yield this.orao.approve(this.vrf.address, oraoToPay);
                }
                else {
                    options = Object.assign(Object.assign({}, options), { value: config.baseFee.sub(subscription.base) });
                }
            }
     ...
     */
    connect(signer) {
        return new OraoVRFClient(this.vrf.connect(signer), this.orao.connect(signer));
    }
}
